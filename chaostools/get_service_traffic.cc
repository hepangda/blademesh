...
int main(int argc, char *argv[]) {
  gflags::SetUsageMessage("./get_service_traffic -chaos_ip <chaos_ip> -q <service name>");
  ...
  auto obj = BladeObject::NewRequest(kCallerServiceName, kChaosSrvAvgBytes);
  ...
  try {
    ...
    if (res != nullptr && res->Contains(kChaosOk)) {
      ...
      cout << "Service[" << FLAGS_q << "] reported " << val << " average bytes(without 40 bytes header) per query." << std::endl;
    } else {
      cout << "No data reported in service[" << FLAGS_q << "]" << endl;
    }
  } catch (...) {
