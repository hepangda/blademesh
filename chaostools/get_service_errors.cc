...
int main(int argc, char *argv[]) {
  gflags::SetUsageMessage("./get_service_errors -chaos_ip <chaos_ip> -q <service name>");
  ...
  auto obj = BladeObject::NewRequest(kCallerServiceName, kChaosSrvError);
  ...
  try {
    ...
    if (res != nullptr && res->Contains(kChaosOk)) {
      ...
      cout << "Service[" << FLAGS_q << "] reported" << val << " errors." << std::endl;
    } else {
      cout << "No errors reported in service[" << FLAGS_q << "]" << endl;
    }
  } catch (...) {
