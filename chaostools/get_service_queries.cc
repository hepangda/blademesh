...
int main(int argc, char *argv[]) {
  gflags::SetUsageMessage("./get_service_queries -chaos_ip <chaos_ip> -q <service name>");
  ...
  auto obj = BladeObject::NewRequest(kCallerServiceName, kChaosSrvQuery);
  ...
  try {
    ...
    if (res != nullptr && res->Contains(kChaosOk)) {
      ...
      cout << "Service[" << FLAGS_q << "]  reported " << val << " queries." << std::endl;
    } else {
      cout << "No queries reported in service[" << FLAGS_q << "]" << endl;
    }
  } catch (...) {
