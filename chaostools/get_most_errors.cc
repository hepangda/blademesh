...
int main(int argc, char *argv[]) {
  gflags::SetUsageMessage("./get_most_errors -chaos_ip <chaos_ip>");
  gflags::ParseCommandLineFlags(&argc, &argv, true);

  auto obj = BladeObject::NewRequest(kCallerServiceName, kChaosMostError);
  ...
  try {
    ...
    if (res != nullptr && res->Contains(kChaosOk)) {
      if (res->Contains(kNoAnswer)) {
        cout << "None service reported errors." << std::endl;
      } else {
        auto name = res->GetString(kChaosName);
        auto val = res->GetI32(kChaosVal);
        cout << "Service[" << name << "] reported " << val << " errors mostly." << std::endl;
      }
    } else {
